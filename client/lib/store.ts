    ...
    (set, get) => ({
      ...
      loadData: async () => {
        try {
          ...
          const [modulesRes, lessonsRes, questionsRes, scaffoldsRes, kcsRes] = await Promise.all([
          ...
          const [modules, lessons, rawQuestions, allScaffolds, kcs] = await Promise.all([
            modulesRes.json(),
            lessonsRes.json(),
          ...
          set({ modules, lessons, questions: rawQuestions, scaffolds: allScaffolds, kcs });

          // Initialize KC masteries if they don't exist
          const state = get();
          if (state.kcMasteries.length === 0 && state.user) {
