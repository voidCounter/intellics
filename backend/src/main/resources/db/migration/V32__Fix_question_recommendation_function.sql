-- Fix ambiguous column reference in recommend_questions function
-- The issue was with the GROUP BY clause in the ready_to_learn CTE
    ...
    ranked AS (
        SELECT wr.question_id,
               ...
               ROW_NUMBER() OVER (
                   ...
                   ORDER BY (wr.weight * (wr.target_mastery - wr.p_mastery) * wr.recency_factor) DESC,
                            wr.question_id
               ) AS kc_rank
        FROM with_recency wr
        WHERE (wr.target_mastery - wr.p_mastery) > 0
          AND wr.recency_factor > 0
    )
