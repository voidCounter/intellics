-- Fix spaced repetition logic and allow practice mode overrides
-- 1. Modify priority calculation to allow reviewing mastered items (spaced repetition)
-- 2. Handle p_max_recency_days <= 0 to support "force practice" mode
...
BEGIN
    ...
    last_correct AS (
        SELECT ui.question_id, MAX(ui.timestamp)::TIMESTAMP AS last_correct_answer
    ...
    last_skipped AS (
        SELECT ui.question_id, MAX(ui.timestamp)::TIMESTAMP AS last_skipped_answer
        FROM user_interactions ui
    ...
    -- 7) Compute recency including skipped questions
    with_recency AS (
        SELECT rtl.question_id,
               rtl.question_text,
               rtl.kc_id,
               rtl.weight,
               rtl.p_mastery,
               ...
               CASE
                   -- If p_max_recency_days is 0 or less, ignore recency (force practice mode)
                   WHEN p_max_recency_days <= 0 THEN 1.0
                   WHEN COALESCE(lc.last_correct_answer, ls.last_skipped_answer) IS NULL THEN 1.0
                   ELSE
                       CASE
    ...
    ranked AS (
        SELECT wr.question_id,
               wr.question_text,
               wr.kc_id,
               -- NEW: Priority includes recency even if mastery gap is 0
               (wr.weight * (GREATEST(0.1, wr.target_mastery - wr.p_mastery) + wr.recency_factor)) AS priority_score,
               wr.p_mastery,
               wr.target_mastery,
               wr.recency_factor,
               wr.last_correct_answer,
               ROW_NUMBER() OVER (
                   PARTITION BY wr.kc_id 
                   ORDER BY (wr.weight * (GREATEST(0.1, wr.target_mastery - wr.p_mastery) + wr.recency_factor)) DESC,
                            wr.question_id
               ) AS kc_rank
        FROM with_recency wr
        WHERE wr.recency_factor > 0 -- Allow items back if recency > 0, even if mastery is full
    )

    SELECT ranked.question_id, 
           ranked.question_text, 
           ranked.kc_id AS primary_kc, 
           ranked.priority_score, 
           (ranked.target_mastery - ranked.p_mastery) AS mastery_gap, 
           ranked.recency_factor, 
           ranked.last_correct_answer
    FROM ranked
    WHERE ranked.kc_rank <= p_per_kc_limit
    ORDER BY ranked.priority_score DESC
    LIMIT p_limit;
END;
$$;
